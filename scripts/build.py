...
def build_standalone():
    ...
    if os.path.exists(dist_dir):
        shutil.rmtree(dist_dir, onerror=lambda func, path, _: (os.chmod(path, stat.S_IWRITE), func(path)))
...
def build_installer():
    ...
    if icon_prop and os.path.exists(icon_prop):
        ...
        # Copy the icon into the dist folder so shortcuts can reference it
        dist_icon_path = os.path.join(dist_dir, "app.ico")
        shutil.copy(os.path.abspath(icon_prop), dist_icon_path)
        app_icon_line = '; IconFilename: "{app}\\app.ico"'
...
[Icons]
Name: "{{group}}\\{app_title}"; Filename: "{{app}}\\{safe_exe_name}"{app_icon_line}
Name: "{{autodesktop}}\\{app_title}"; Filename: "{{app}}\\{safe_exe_name}"{app_icon_line}; Tasks: desktopicon
